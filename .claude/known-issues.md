...
**Status**: Zero tests across all three services

**Scope**:
- Backend: no Mocha/Jest/Supertest tests for any endpoint
- Frontend: no React Testing Library or Jest component tests
- ML API: no pytest unit tests for `calculate_risk_score`, `run_maintenance_prediction`

**Impact**: CI cannot verify correctness. Any refactor risks silent regressions.
`render.yaml` has no test step — deploys blind.

**Recommended starting points**:
- Backend: `npm install --save-dev jest supertest` → test `/api/auth/login` and `/api/reactors/stream`
- ML API: `pip install pytest` → test `calculate_risk_score` with known inputs
- Frontend: Create React App ships with Jest — test SocketContext and RiskGauge
